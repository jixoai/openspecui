...
/**
 * Mirror a disk command file written by an earlier line: the official 1.12
...
describe('getToolInitStates', () => {
  ...
  beforeAll(async () => {
    ...
    if (!loaded) throw new Error('Pinned OpenSpec 1.12 command generator is unavailable.')
  ...
  it('flags below-admitted generator versions stale inside an admitted session', async () => {
    ...
    for (const belowAdmitted of ['1.11.0', '1.10.2', '1.9.0'] as const) {
      ...
      expect(state).toMatchObject({
        readiness: 'initialized',
        status: 'stale-version',
        generatedByVersion: belowAdmitted,
        issues: ['stale-version'],
      })
    }
  })
  ...
  it('treats the OpenCode provided-arguments injection line as generator-owned equivalence', async () => {
    ...
    // The official 1.12 body already carries the adapter-injected passthrough line; the
    // disk file models the pre-injection body from an earlier line.
  ...
  it('retains the selected registry across replacement emissions', async () => {
    ...
    try {
      ...
      expect(antigravity?.skillsScope).toEqual({ kind: 'project', skillsDir: '.agents' })
  ...
  it('projects no per-tool states for retired below-range series selections', async () => {
    ...
    for (const retired of ['1.10.1', '1.11.2'] as const) {
      const states = await getToolInitStates(tempDir, {
        delivery: 'both',
        workflows: ['explore'],
        generatorVersion: retired,
        registry: selectAgentDeliveryRegistry(retired),
      })
