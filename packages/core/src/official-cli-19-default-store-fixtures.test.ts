 ...
 * 1. Execute the pinned OpenSpec 1.9.0 CLI against machine `defaultStore` root selection.
...
describe('pinned OpenSpec 1.9 defaultStore fixtures', () => {
  ...
  it('reports global_default only when the configured Store is selected effectively', async () => {
    fixtureRoot = await createPinnedFixtureRoot('cli-190-default-store')
    ...
    const doctorResult = await runPinnedOpenspec('1.9.0', ['doctor', '--json'], scratch, env)
    ...
    const contextResult = await runPinnedOpenspec('1.9.0', ['context', '--json'], scratch, env)
  ...
  it('keeps absent and stale fallback failures distinct', async () => {
    ...
    const absent = await runPinnedOpenspec('1.9.0', ['doctor', '--json'], scratch, env)
    ...
    expect(configured.exitCode, configured.stdout + configured.stderr).toBe(0)
    const stale = await runPinnedOpenspec('1.9.0', ['doctor', '--json'], scratch, env)
