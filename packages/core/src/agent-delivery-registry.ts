 ...
 * registry order (`codex`, `zed`, `agents` on the admitted line).
 *
...
export const AGENT_DELIVERY_REGISTRY: ToolConfig[] = [
  ...
  },
  {
    ...
    // tree (adapter-backed tools are excluded from skills-writer arbitration).
    // Cleanup stays scoped to pre-opsx filenames under the former `.agent` root:
    // upstream never lists the shared root because users may keep their own files
    // there. (The retired 1.10 line treated `.agent` as the current root; it left
    // the admitted window with OpenSpecUI 12 and selects no inventory, so the
    // per-series override mechanism stays free for a future widened window.)
    name: 'Antigravity',
    value: 'antigravity',
    available: true,
  ...
  {
    // SourceCraft Code Assistant enters with OpenSpec 1.12. Skills live under
    // `.codeassistant/skills/` and are referenced in natural language (upstream
    // `NATURAL_LANGUAGE_SKILL_TOOLS`, like Rovo Dev) — unlike rovodev it also
    // ships command files at `.codeassistant/commands/opsx-<id>.md` with YAML
    // frontmatter carrying only `description`. Default `.codeassistant`
    // detection root, no IDE restart fact, no legacy roots, no migrations.
    ...
    capability: 'adapter-backed',
    command: command('.codeassistant/commands/opsx-{workflow}.md', yamlMarkdown('description')),
  },
  {
    name: 'Qoder',
    value: 'qoder',
