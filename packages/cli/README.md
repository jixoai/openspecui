...
1. Document the npm-facing surface of the current openspecui release line (v9) only.
...
OpenSpecUI 9 gives OpenSpec projects a reactive dashboard, an objective change workflow, a config workbench, real terminals, and static export — while the OpenSpec CLI stays the source of truth for every workflow fact.

## Requirements

- **OpenSpec CLI**: `>=1.8.0 <1.10.0` — the stable 1.9 line is current and recommended; stable 1.8.x is supported non-current. Older lines, prereleases, and `>=1.10.0` are blocked by default.
- **Node.js**: `>= 20.19.0`
...
- **OPSX change workflow** — kanban lanes over tracked task phases with Continue / Fast-forward / Apply / Verify / Archive operators driven by live CLI evidence
- **Dashboard** — active changes with CLI-owned applying progress and a curated Code Git snapshot
- **Config workbench** — route-backed owners for project binding, active root, environment globals, and schemas, plus an adaptive setup guide
- **Agent delivery** — the CLI-owned registry projects per-version agent commands and skills
