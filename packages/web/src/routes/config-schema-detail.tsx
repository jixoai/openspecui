 ...
 * 1. Own one route-selected Schema's inspection, creation, file editing, and deletion workflows.
 * 2. Preserve source-specific read-only behavior behind current Root and Config Bundle authority.
 * 3. Represent independent Schema catalog and selected-file projections without false empty conclusions.
 *
 ...
 * Owner Config-workbench decision (2026-08-01): Schema entities belong to `/config/schemas/:id`, never the fixed-owner navigation plane.
 */
...
/** Route adapter for one encoded Schema identity. */
export function ConfigSchemaDetail() {
  const { schemaId } = useParams({ strict: false })
  const navigateByHref = useVTHrefNavigate()
  return (
    <ConfigSchemaWorkspace
      schemaId={schemaId}
      onNavigate={(href) => void navigateByHref({ href })}
    />
  )
}

/** Testable live/static workspace for one explicit Schema identity. */
export function ConfigSchemaWorkspace({
  onNavigate,
  schemaId,
}: {
  ...
  const {
    ...
    error: schemasError,
    authority: configBundleAuthority,
  } = useOpsxConfigBundleSubscription()
  const configBundleAuthorityRef = useRef(configBundleAuthority)
  configBundleAuthorityRef.current = configBundleAuthority
  ...
  const schemaCanEdit =
    !isStatic &&
    !rootAction.disabled &&
    configBundleAuthority.state === 'current' &&
  ...
  const requireRootActionReady = useCallback(() => {
    ...
    if (configBundleAuthorityRef.current.state !== 'current') {
      throw new Error('Schema configuration is refreshing; mutations remain read-only.')
    }
  }, [])
  ...
  const handleConfirmAddSchema = useCallback(() => {
    ...
    createSchemaMutation.mutate(input, {
      onSuccess: () => {
        ...
        onNavigate(`/config/schemas/${encodeURIComponent(normalizedName)}`)
      },
    })
  }, [
  ...
  const handleConfirmDeleteSchema = useCallback(() => {
    ...
    deleteSchemaMutation.mutate(undefined, {
      onSuccess: () => {
        ...
        onNavigate('/config/schemas')
      },
    })
  }, [deleteSchemaMutation, onNavigate, schemaCanEdit])
  ...
  const schemaTabContent = (
    <section data-schema-detail-content="true" className="min-w-0">
  ...
  return (
    <ConfigWorkbenchPage
      ...
      header={
        <ConfigOwnerHeader
          backHref="/config/schemas"
          backLabel="Schemas"
          title={selectedSchema ? `Schema: ${selectedSchema}` : 'Schema'}
          description="Inspect resolved schema facts, preview artifacts, and edit writable physical files."
          icon={<Layers className="h-6 w-6 shrink-0" aria-hidden />}
        />
      }
    >
      <div className="space-y-4">
        ...
        >
          <div className="space-y-4 text-sm">
            ...
            {schemaActionError && (
              <div className="text-destructive text-xs">{schemaActionError}</div>
            )}
          </div>
        </Dialog>
        ...
        >
          <div className="space-y-2 text-sm">
            <p>
              ...
              <code className="bg-muted rounded px-1">{selectedSchema}</code> and all of its
              template files.
            </p>
        ...
        <div data-schema-workspace-status="true" className="space-y-4">
          {schemasError ? (
            <div
              role="alert"
            ...
            >
              ...
              <div>
                ...
                <p>{schemasError.message}</p>
              </div>
            </div>
          ) : null}
          {schemaCatalogInitialLoading ? (
            <section
              aria-label="Loading Schema detail"
          ...
          {schemas !== undefined && schemas.length > 0 && !selectedSchemaInfo ? (
            <div role="alert" className="text-destructive rounded-lg border p-4 text-sm">
              Schema {selectedSchema || '(missing id)'} was not found.
            </div>
          ) : null}
          {selectedSchemaInfo ? schemaTabContent : null}
        </div>
      </div>
    </ConfigWorkbenchPage>
  )
