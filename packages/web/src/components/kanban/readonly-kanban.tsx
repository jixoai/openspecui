 ...
 * 3. Own a 1/2/4-column container-responsive topology with independent lane scrolling.
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          '@[32rem]:grid-cols-2 @[64rem]:grid-cols-4 grid w-full min-w-0 max-w-full grid-cols-1 gap-3 overflow-x-clip pb-6',
      ...
      >
        {KANBAN_LANES.map((lane) => {
          ...
          return (
            <section
              key={lane.id}
              className="border-border/70 flex min-h-0 min-w-0 flex-col overflow-hidden border-t"
            >
              ...
              <div
                data-kanban-lane-scroll={lane.id}
                className="scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[color-mix(in_srgb,currentColor,transparent_78%)] min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain pb-1 pr-1"
              >
                {pending ? (
