 ...
 * 1. Project exact Active Root owner/file/revision and explicit replacement lifecycle.
...
export function ActiveRootConfigSection({ isStatic }: { isStatic: boolean }) {
  ...
  return (
    ...
    >
      <section
        {...guideAnchor}
        className="border-border bg-card flex min-h-0 min-w-0 flex-1 flex-col gap-3 overflow-hidden rounded-lg border p-4"
      >
          ...
          <div className="min-w-0 flex-1">
            ...
            {config.owner ? (
              <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1.5 text-xs">
                ...
                {config.revision ? (
                  <InformationBadge
                    ariaLabel="Active Root config revision"
                    tooltip={config.revision}
                  >
        ...
        {config.diagnostics.length > 0 ? (
          <div
            role={
              config.diagnostics.some(({ severity }) => severity === 'error') ? 'alert' : 'note'
            }
        ...
        {config.exists || isEditing ? (
          <div
            ...
            aria-label={
              mode === 'structured'
                ? 'Structured Active Root editor'
                : 'Raw YAML Active Root editor'
            }
