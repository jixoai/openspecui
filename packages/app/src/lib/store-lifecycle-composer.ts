 ...
 * 4. Project backend-owned HTTP admission evidence until the lifecycle ledger carries the same request.
 *
...
export function createStoreLifecycleComposer(dependencies: StoreLifecycleComposerDependencies) {
  ...
  const admissionRecords = new Map<string, StoreMutationEnvelope>()
  ...
  const project = (projection: MutationLocatorProjection | null): StoreLifecycleProjection => {
    const view = projectStoreLifecycle(projection)
    const ledgerIds = new Set(projection?.records.map(({ requestId }) => requestId) ?? [])
    const admissions = [...admissionRecords.values()].filter(
      ({ requestId }) => !ledgerIds.has(requestId)
    )
    return {
      ...view,
      active: [
        ...admissions.filter(({ status }) => status === 'accepted' || status === 'running'),
        ...view.active,
      ],
    }
  }
  ...
  return {
    ...
    registerAdmission(apiBaseUrl: string, admission: StoreMutationEnvelope): void {
      const normalized = normalizeHostedApiBaseUrl(apiBaseUrl)
      if (!normalized || normalized !== locator || admission.requestId.length === 0) return
      if (refreshedRequestIds.has(admission.requestId)) return
      const currentRecord = hasCurrentProjection
        ? lastCurrentRecords.get(admission.requestId)
        : undefined
      if (currentRecord && TERMINAL_STATUSES.has(currentRecord.status)) {
        settledRequestIds.add(admission.requestId)
        refreshedRequestIds.add(admission.requestId)
        pullSettledProjection(lastCurrentTabs)
        return
      }
      pendingAdmissions.add(admission.requestId)
      admissionRecords.set(admission.requestId, admission)
      dependencies.onAdmissionChange?.()
    },
    project,
    observe(
      ...
      if (!projection?.current || projection.apiBaseUrl !== locator) return project(projection)
      ...
      for (const requestId of lastCurrentRecords.keys()) admissionRecords.delete(requestId)
