 ...
 * 2. Prove retired (1.11/1.12), older, and unavailable-CLI sessions never construct the command (transport count 0).
...
describe('readChangeDiffEvidence', () => {
  it('projects MODIFIED delta diff and warning with CLI provenance for an admitted 1.13 session', async () => {
    ...
    const projection = await readChangeDiffEvidence(
      deps,
      rootContextFixture({ version: '1.13.0' }),
      'add-search'
    )
  ...
  it('never constructs the command on a retired 1.12 session and projects capability unavailability', async () => {
    const deps = depsFixture({
      kind: 'executed',
      result: commandResultFixture(successPayload()),
    })
  ...
  it('projects root loss as unavailability without constructing the command', async () => {
    ...
    const projection = await readChangeDiffEvidence(
      deps,
      rootContextFixture({ version: '1.13.0', planningRoot: null }),
      'add-search'
    )
