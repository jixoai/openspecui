...
import { assertMatchingDashboardSummaryRead } from '../bench/live-projection-loading-summary.js'
...
describe('live projection loading Summary pair', () => {
  it('rejects a typed read whose identity differs from the wake', () => {
    ...
    const read: DashboardSummaryRead = {
    ...
    expect(() => assertMatchingDashboardSummaryRead(wake, read)).toThrow('identity mismatch')
  })
  ...
  it('rejects a typed read whose work generation differs from the wake', () => {
    ...
    expect(() => assertMatchingDashboardSummaryRead(wake, read)).toThrow('generation mismatch')
  })
})
