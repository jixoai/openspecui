...
5. Record the Web-first execution replan: Phase 1 (P2/P3/P4/P5-static on v1 transport) advances now; Phase 2
   (P1 Server v2 protocol + adapter upgrade + closing gates) is gated on the owner's P0 Server emission
   regression repair.
...
Reviewer replan decision (2026-07-24): "先做 Web 侧(P2/3/4)，我会同步去修复 P0，等完成后你再看看能不能做 P1" (Web-first; P1 gated on P0 repair).
Regression blocker (2026-07-23, commit 61612c3): watcher-pool async invalidate(['context']) x planning-root-service snapshot cache invalidationKey gate drops reactive emissions; blocks pnpm test:ci.
-->

## Checkpoint State

This is a planning-complete, pre-apply checkpoint. No source, test, CSS, static-export, PR, or release gate for this Change has run. Completion of this artifact schema is not implementation completion: delta specifications, strict validation, all five implementation packages, CI, PR review, and owner browser acceptance remain open.
...
Execution order (reviewer replan, 2026-07-24): **Phase 1 (worker advances now)** = P2 -> P3 -> P4 +
P5-static/a11y on the current v1 transport; **Phase 2 (after owner confirms P0 green)** = P1 Server v2
protocol + adapter input upgrade + P5 closing gates. The numbered order below is unchanged; these phase
annotations describe implementation timing, not checkpoint numbering. See `GOAL.md` "Execution Order" for
the decoupling design and the P0 regression blocker evidence.

- [ ] 2.1 P1 started from the approved plan: add the version-2 invalidation/pull contract and a typed Web reducer without v1 compatibility glue or a parallel client truth source. *(Phase 2, gated on P0)*
- [ ] 2.2 P1 red evidence recorded at the public adapter boundary: current A invalidates, B rebinds, A pull resolves late, and pre-fix behavior can publish A incorrectly. *(Phase 2)*
- [ ] 2.3 P1 green and mutation-resistance evidence recorded: only matching identity/generation commits; coalescing is bounded; user retry bypasses it; removing the acceptance gate fails the test. *(Phase 2)*
