...
1. Capture the owner's full-surface waiting-experience request without changing the pending Kanban layout work.
...
本 Change 统一整改当前 Web 的路由页面、详情区、Dialog、Popover、Drawer、设置分区和操作控件的等待体验；不改变既有页面的信息架构、导航或布局。它沉淀一个可组合的实时状态组件族，并迁移全部现有异步表面。
...
- 不手写脆弱的滚动锚定、焦点迁移、JS 动画循环或跨页面预热；隐藏/未挂载路由不为视觉新鲜度持续订阅或拉取。
...
- 所有当前路由与叠层异步表面都有迁移清单；每项明确现有事实源、初始/部分/重验/失败/空态、可写性、草稿冲突策略与不改布局的边界。
