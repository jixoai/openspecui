...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | `packages/core/src/openspec-compat.ts` + compatibility UI copy (`cli-health-gate.tsx`, settings diagnostics, `use-cli-runner`) + `scripts/setup-example.ts` mirror                                  | `1.12.0-rc.1` admitted or `1.11.0` still admitted; a `1.10.x` executable still admitted; setup-example still pins `1.11`                                                                                                                                              | 1.12.x current and recommended; 1.10.x/1.11.x rejected with actionable copy naming `>=1.12.0 <1.13.0`; both constant sites agree. Run `openspec-compat.test.ts` first.                                                                                                                                                                                                                                                                                                                                                      |
...
| 4     | Real fixture harnesses: core `package.json` devDeps (`openspec-cli-112` alias; keep `-111`), `__tests__/official-cli-v12-fixtures.ts` + `official-cli-v12-*` suites, bins map, `upstream-contract-regression.test.ts` re-pin (already on this branch) | a hand-authored payload proves a 1.12 behavior; the 1.11 boundary negative dropped; upstream source assertions still referencing `a0ddb60`                                                                                                                                                       | pinned 1.12.0 executables prove the findings envelope (populated/empty/request-error), the merge-conflict INFO class, the unchanged full report, codeassistant physical layout, `.gitkeep` anchors, restart-hint wording, and JSON stream purity; pinned 1.11.0 proves `--report` rejection. Run matrix tests independently first.                                                                                                                                                                                          |
...
| 6     | Server read-model + Web surfaces: validation-evidence owners, `/config/agents` inventory, `cli-health-gate`/settings copy                                                                          | INFO rendered as generic warning noise or dropped; findings surface hides full-run totals behind `returnedItems`; registry empty for an admitted 1.12 session; blocked-1.11 dialog copy still naming v11 ranges                                                                 | an end-to-end typed chain: Core findings contract -> Server projection -> validation evidence surface rendering INFO as a first-class informational class with both `returnedItems` and preserved totals and CLI provenance; `/config/agents` shows the SourceCraft inventory row with natural-language skill reference display; mismatch dialog names `>=1.12.0 <1.13.0`. Run focused Web component tests.                                                                                                                      |
| 7     | Release preparation: Changesets (major v12), `README.md` + `README-zh-*.md` + `packages/cli/README.md` under the release README law, package build/pack, AGENTS.md architecture-decision entry (including correcting the two stale v1.7.0 submodule-pin statements) | packed CLI/Web assets still carry the v11 gate; README version table still describing 1.10/1.11 as current; AGENTS.md still asserting a v1.7.0 reference pin                                                                                                                                                                       | one v12 major Changeset for the fixed group; both READMEs state the v12 line (repository README keeps the full version table with v11 archived); pack evidence proves source and distribution shape agree; AGENTS.md records the v12 window decision. No publish/release action.                                                                                                                                                                                                                                             |
...
5. Registry: the `'1.12'` snapshot inherits every 1.11 physical fact (antigravity `.agents`-current with
   ...
   future widening; do not collapse it into a single flat registry.
...
9. Spec hygiene rides with this Change: delta Specs advance stale line wording (`Batch Status Envelope
   ...
   `Pinned Workflow Fixtures Are Executable`) and rename the version-bound `OpenSpecUI 11 Agent Delivery
   Inventory`. One delta file uses at most one `## MODIFIED Requirements` section (the upstream parser
