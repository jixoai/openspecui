...
CLI 1.12 validate --report findings --json (requires explicit bulk scope; no item name; no archived+active mix)
  ...
  full report     = envelope/schema-compatible with 1.11 ({ items, summary, version:'1.0', root });
                   1.12 may add INFO issues and runtime-dependent values, so byte-identity is not claimed
...
Agent delivery 1.12
  + codeassistant (SourceCraft Code Assistant): skillsDir .codeassistant, NOT requiresIdeRestart,
    commands .codeassistant/commands/opsx-<id>.md, skills .codeassistant/skills/openspec-*/SKILL.md
    (artifact count is profile-dependent: 6+6 under the isolated default core profile; fixtures pin the profile)
...
init 1.12
  ...
  never overwrites files or follows marker symlinks); init passes an empty ledger — the store-root callers
  own the created-path ledger
