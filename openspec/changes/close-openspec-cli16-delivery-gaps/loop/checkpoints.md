...
- [x] 6.3d Carried the parent CLI runtime's already resolved `webAssetsDir` through `WorktreeInstanceManager`, checked worker data, and a consumed private process environment value. Both real child fixtures serve distinct markers from minimal physical asset roots in a clean checkout; bypassing either exact handoff makes its corresponding fixture fail with `Web assets not found`. Nested process-child Managers retain the same root; no public CLI option or target-worktree asset discovery was added.
