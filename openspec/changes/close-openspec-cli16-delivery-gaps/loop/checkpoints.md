...
- [ ] 6.3 Clean source-resolution correction `7f18cd9` is independently accepted: isolated no-`dist` App typecheck is green, restoring the old partial App `paths` makes the same fixture red, and restoring the correction is green again. Reclose after one complete local gate replay against the corrected head.
