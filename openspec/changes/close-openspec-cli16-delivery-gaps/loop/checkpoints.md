...
- [ ] 6.3 Reconcile local and clean CI after accepted 6.3b and the independent 6.3c Web Router event-lifecycle correction; then replay the complete local gate once without generated Web output assumptions or widened timeouts.
...
- [ ] 6.3c Make the real Spec Router handoff fixture await its asynchronous `onResolved` lifecycle before asserting the emitted path. The production route, document request, cache prime, transition, and location already converged; do not change production navigation or hide a missing event. Removing the real event subscription must still fail the named fixture.
