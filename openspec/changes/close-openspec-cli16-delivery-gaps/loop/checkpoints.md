...
- [x] 6.3 Reconcile the corrected local candidate only after accepted 6.3d: format, lint, all 15 workspace typechecks, complete unit tests, component browser lanes, diff check, and strict Change validation must pass without generated Web output assumptions or widened timeouts.
...
the Web unit project to `50%` workers, the standard Web package command passed `154/154` files and `989/989` tests
