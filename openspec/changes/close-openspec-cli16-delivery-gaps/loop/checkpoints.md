...
- [ ] 6.3 Reconcile local and clean CI only after the repeated full-suite Tool observation failure in 6.3b is corrected and independently accepted; then replay the complete local gate once without generated Web output assumptions or widened timeouts.
...
- [ ] 6.3b Correct the repeated clean-CI Tool `commands/update` observation failure. Two independent CI heads miss the same second creation while isolated dependency evidence stays current, so the worker must reduce Tool artifact observation/cache-reset fanout at the real projection owner, preserve expected/unexpected/legacy semantics, and prove the correction without timeout inflation or a speculative generic-kernel rewrite.
