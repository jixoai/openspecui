...
- [ ] 6.3d Carry the parent CLI runtime's already resolved `webAssetsDir` through `WorktreeInstanceManager`, checked worker data, and a consumed private process environment value. Keep the accepted worker/process branch fixtures, then add one typechecked real `startServer -> tRPC Git switchWorktree -> child Server` fixture that serves the parent-owned marker. Replacing the exact upstream `startServer -> Manager` argument with an invalid root must fail that named fixture at child bootstrap. No public CLI option, target-worktree asset discovery, generated-output fallback, or hand-authored Manager injection may satisfy the upstream proof.
