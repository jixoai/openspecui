...
| Full gates require one corrected-head replay.                       | Clean CI runs `30163937799` and `30165778790` both failed the same Tool public subscription stage. The accepted `ace53a5`/`a9214ed` correction replaces per-artifact observation with retained directory inventories, reducing measured states/subscriptions/polls from `720/401/720` to `60/60/0`; the real Server path now covers observation-root creation, deletion, and recreation. | 6.3b is focused-review complete. Replay the complete local gate once on the corrected head; only then update PR #207. Browser automation remains preparation evidence and manager walkthroughs remain separate. |
