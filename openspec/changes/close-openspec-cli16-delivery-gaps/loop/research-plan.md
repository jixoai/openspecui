...
| Full gates are currently not green.                                 | The physical Web asset correction removed the prior product-chain failure, but clean CI runs `30163937799` and `30165778790` both failed the same Tool public subscription stage: after the `explore.md` replacement emission, Launch `update.md` creation was not observed within four fallback cycles. Isolated evidence proves the dependency remains registered; it does not reproduce full Server-suite scheduler pressure. | Reopen 6.3/6.3b. Diagnose and reduce the Tool artifact observation fanout at its production owner; do not widen the timeout or change the generic reactive kernel without a separate fixed-point red. Browser shards remain skipped consequences, not browser failures. |
