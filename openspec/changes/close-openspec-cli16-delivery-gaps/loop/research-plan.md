...
| Corrected-head full gates exposed one independent Web test race.    | 6.3b is accepted. The first complete unit replay had one Server teardown-hook timeout, but the exact file passed `6/6` and the next full Server package passed `74` files / `493` tests, so no timeout or Server change is authorized. The second replay reached Web and found the real Spec Router at the target path with document/cache/transition evidence while its asynchronous `onResolved` capture was still `[]`; the exact file passed `2/2`. | Split 6.3c as a test-lifecycle correction: await the real Router event before assertion and retain mutation evidence that removing the subscription fails. If Server teardown repeats after that correction, reopen it independently; do not mix it with Tool or Web navigation. |
