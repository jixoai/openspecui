...
| Corrected-head gates require a post-6.3c replay.                    | 6.3b is accepted. One Server teardown-hook timeout did not repeat: the exact file passed `6/6` and the next full Server package passed `74` files / `493` tests, so no timeout or Server change is authorized. Accepted 6.3c now awaits the real Spec Router `onResolved` event; removing the actual subscription makes the owned fixture fail while the detail remains rendered. | Replay the complete local gate once from `39743c3`; if Server teardown recurs, reopen it independently. Do not mix it with Tool or Web navigation, and do not update PR #207 before every local gate passes. |
