...
- [ ] 6.16 Every network-triggering control binds loading/disabled state and every page covers empty/loading/loaded/updating/error topologies
  ...
  - `6.16-D` implemented, awaiting independent review: ArchiveList no longer adds an artificial rAF gate;
    synchronous real-component evidence renders resolved data immediately and preserves the initial
    unknown-data Loading branch. Archive error and cached-updating topology remain later work, so this
    slice cannot close 6.16.
