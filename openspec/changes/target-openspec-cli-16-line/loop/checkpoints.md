...
- [ ] 6.16 Every network-triggering control binds loading/disabled state and every page covers empty/loading/loaded/updating/error topologies
  ...
  - `6.16-F2` corrected and awaiting independent review: only Archive uses the typed event helper;
    retained `[]` during Updating now renders only the Updating fact, while the ordinary current-empty copy
    remains available after settlement. The exact empty-branch guard has direct mutation evidence.
