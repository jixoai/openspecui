...
- [ ] 6.16 Every network-triggering control binds loading/disabled state and every page covers empty/loading/loaded/updating/error topologies
  ...
  - `6.16-E` planned: render real no-data and retained-data Archive transport errors without false empty or
    current-success claims. Archive updating remains blocked on a shared production subscription signal;
    a component-only `data + isLoading=true` mock is not acceptance evidence.
