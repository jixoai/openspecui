...
- [ ] 6.16 Every network-triggering control binds loading/disabled state and every page covers empty/loading/loaded/updating/error topologies
  ...
  - `6.16-L` implemented, awaiting independent review: Config Schema catalog/tab-inventory topology consumes
    ...
    remains open.
  - `6.16-M` planned: extract the shared Web subscription lifecycle owner before another page adopts
    lifecycle behavior. The internal owner must make cached snapshots, effect-generation identity,
    subscription cleanup, and static-loader publication one physical contract across ordinary, reactive, and
    authoritative hooks. It must close ordinary-hook late A -> B/cache publication without changing the
    existing ordinary rebind policy, reactive Updating contract, or authoritative gate.
